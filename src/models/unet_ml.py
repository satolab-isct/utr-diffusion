...
# works for both single-label and double-label
class UNet_Multi_Labels(nn.Module):
    def __init__(
        self,
        dim: int,
        ...
        self.label_emb_list = nn.ModuleList(
            [nn.Embedding(num_classes + 1, time_dim) for _ in range(num_labels)]
        )
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor = None, classes: torch.Tensor = None):
        ...
        if classes is not None:
            ...
            for i in range(self.num_labels):
                class_i = classes[:, i]
                emb_i = self.label_emb_list[i](class_i)
                label_embs.append(emb_i)
            label_emb_combined = sum(label_embs)
            t_start += label_emb_combined
            t_mid += label_emb_combined
            t_end += label_emb_combined
            t_cross += label_emb_combined
