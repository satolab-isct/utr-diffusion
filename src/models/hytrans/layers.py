...
def Downsample(dim: int, dim_out: int | None, is_last):
    # if is_last:
    #     downsample = nn.Conv2d(dim, default(dim_out, dim), 3, 2, 1)
    # else:
    downsample = nn.Conv2d(dim, default(dim_out, dim), 4, 2, 1)
    return downsample
...
class Block(nn.Module):
    def __init__(self, dim: int, dim_out: int, groups: int = 8, dropout: float = 0.0) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, scale_shift: torch.Tensor | None = None):
        ...
        x = self.act(x)
        return self.dropout(x) ## added dropout layer to the basic Block


class ResnetBlock(nn.Module):
    def __init__(self, dim: int, dim_out: int, *, time_emb_dim: int | None, groups: int = 8, dropout: float = 0.0) -> None:
        super().__init__()
...
class LearnableContext(nn.Module):
    def __init__(self, input_dim, num_tokens, output_dim):
        super().__init__()
        self.linears = nn.ModuleList([
            nn.Sequential(nn.Linear(input_dim, 2 * output_dim), nn.SiLU())
            for _ in range(num_tokens)
        ])

    def forward(self, x):  # x: [B, 2*D]
        return torch.stack([linear(x) for linear in self.linears], dim=1)  # [B, N, D]

class LearnableContextPlus(nn.Module):
    def __init__(self, in_dim, out_dim, num_tokens, dropout=0.1):
        super().__init__()
        self.A_pathway = nn.ModuleList([
            nn.Sequential(
        ...
        if in_dim != out_dim:
            ...
            self.B_pathway = nn.ModuleList([
                nn.Identity() for _ in range(num_tokens)
            ])

        self.layer_norm = nn.LayerNorm(out_dim)

    def forward(self, x):
        res = torch.stack([layers(x) for layers in self.B_pathway], dim=1)
        x = torch.stack([layers(x) for layers in self.A_pathway], dim=1)
        x += res
