...
# basically work same as diffusion(discrete version),
# available for continuously and multi-labels (or single label) data
class Diffusion_Continuous_Multi_Labels(nn.Module):
    def __init__(
        self,
        model,
        timestep: int = 50,
    ...
    def p_sample_guided(self, x, classes, t, t_index, context_mask, cond_weight):
        ...
        preds = self.model(x_double, time=t_double, classes=classes, context_mask=context_mask)
    ...
    def p_losses(self, x_start, t, classes, noise=None, loss_type="huber"):
        ...
        # for multi-labels
        if classes.ndim == 1: # for single-label case
            classes = classes.unsqueeze(1)
        else:# for multi-labels case
            context_mask = context_mask.unsqueeze(1).repeat(1, classes.shape[1])
        # Mask for unconditional guidance
        predicted_noise = self.model(x_noisy, time=t, classes=classes, context_mask=context_mask)
