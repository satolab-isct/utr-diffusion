...
# this repaint sampler works for discrete single-label generation
class RePaintSampler:
    def __init__(self, diffusion, sample_bs, seq_len, cond_weight, num_class, return_all=True):
    ...
    def p_resample(self, gt, mask):
        ...
        if gt.ndim != 4:
            gt = gt.repeat(self.sample_bs * self.num_class, 1, 1, 1).to(self.device)
        else:
            gt = gt.to(self.device)

        if mask.ndim != 4: # without batch
            mask = mask.repeat(self.sample_bs * self.num_class, 1, 1, 1).to(self.device)
        else:
