...
class RePaint_Amino_Relaxed:
    def __init__(self,
        ...
        self.reanchor_threshold = 1 - reanchor_threshold
    ...
    def p_resample(self, gt, mask, tgt_aminos, pos_list):
        ...
        result = {'samples': [], 'forward_steps': [], 'backward_steps': [], 'total_steps': [], 'gt': [], 'gt_noised': []}
        if self.return_all:
            for idx, (sample, forward_step, backward_step, gt, gt_noised) in enumerate(
                    self.p_resample_loop(gt, mask, labels, all_amino_images, pos_list)):
                ...
                result['total_steps'].append(forward_step + backward_step)
                result['gt'].append(
                    gt.cpu().detach().to(torch.float16).numpy() if gt is not None else None
                )
                result['gt_noised'].append(
                    gt_noised.detach().cpu().to(torch.float16).numpy() if gt_noised is not None else None
                )

        else:
            for sample, _, _, _, _ in self.p_resample_loop(gt, mask, labels, all_amino_images,
                                                                            pos_list):
    ...
    def p_resample_loop(self, gt, mask, labels, tgt_amino_images, amino_pos):
        ...
        gt, gt_noised = None, None
        for t_last, t_cur in time_pairs:
            if t_cur < t_last:  # backward process
                backward_step_count += 1
                total_step_count += 1
                # early stopping
                in_early_phase = total_step_count < self.reanchor_threshold * len(time_pairs)
                do_reancher = (not in_early_phase) or (in_early_phase and backward_step_count % self.reanchor_interval == 0)
                if do_reancher:
            ...
            else: # forward process (renoising process)
                forward_step_count += 1
                total_step_count += 1
                image = self.noise_step(x_t_m1=image, t=t_last)

            yield image, forward_step_count, backward_step_count, gt, gt_noised
