...
class RePaint_Amino_Continuous_Multi_Labels:
    ...
    def p_resample(self, gt, mask, tgt_aminos, pos_list):
        ...
        if self.return_all:
            for idx, (sample, forward_step, backward_step) in enumerate(self.p_resample_loop(gt, mask, labels, all_amino_images, pos_list)):
...
def choose_codon_by_strategy(query: torch.Tensor, candidates: torch.Tensor, strategy:str = 'wasserstein') -> torch.Tensor:
    """
