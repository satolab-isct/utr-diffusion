...
from src.utils.sample_util import convert_sample_to_fasta, create_sample, extract_motifs
...
def generate_similarity_metric(seqs):
    """Capture the syn_motifs.fasta and compare with the  dataset motifs"""
    nucleotides = ["A", "C", "G", "T"]
    seqs_to_hotencoder = [one_hot_encode(seq.split('\n')[1], nucleotides, 50).T for seq in seqs]
...
def calculate_mean_similarity(database, input_query_seqs, seq_len=50):
    for x in tqdm(input_query_seqs):
        final_base_max_match = np.mean([get_best_match(database, x)])
    return final_base_max_match / seq_len


def generate_similarity_using_train(X_train_in, generated_seqs):
    convert_X_train = X_train_in.copy()
    convert_X_train[convert_X_train == -1] = 0
    generated_seqs_to_similarity = generate_similarity_metric(generated_seqs)
